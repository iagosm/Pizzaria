<?php
  ...
  if($method === "GET"){
    ...
    //precisamos saber quais são os pedidos que temos, selecionar a borda, massa, sabor e outras operações
    $pedidosQuery = $conn->query("SELECT * FROM pedidos");

    $pedidos = $pedidosQuery->fetchAll(); // Irá trazer apenas ids
    ...
    foreach($pedidos as $pedido){
      ...
      // Adicionar o array pizza ao array das pizzas

      array_push($pizzas, $pizza); // Receber todas as pizzas no front-end atraves desse variavel
    ...
    $status = $statusQuery->fetchAll(); // Receber todos os status no front-end atraves desse variavel
    ...
    if($type === "delete") {
        ...
        $deleteQuery->bindParam(":pizza_id", $pizzaId, PDO::PARAM_INT); //Filtrando com paramint para saber se é um inteiro
